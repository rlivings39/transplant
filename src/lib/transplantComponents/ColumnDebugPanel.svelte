                    ...
                    <tbody>
                        {#each columns as column}
                            <tr onclick={() => selectedColumn = column} style="cursor: pointer;">
                                ...
                                <td>{column.validation?.isValid ? '✓' : '✗'}</td>
                            </tr>
                        {/each}
                    </tbody>
                </table>
                    ...
                    {#if expandedSections.basic}
                        <div class="property-list">
                            ...
                            <div class="property-item">
                                ...
                                <span class="property-value property-{selectedColumn.isToggled}">{selectedColumn.isToggled}</span>
                            </div>
                            <div class="property-item">
                                <span class="property-name">Is Formatted:</span>
                                <span class="property-value property-{selectedColumn.isFormatted}">{selectedColumn.isFormatted}</span>
                            </div>
                            <div class="property-item">
                                <span class="property-name">Is Mapped:</span>
                                <span class="property-value property-{selectedColumn.isMapped}">{selectedColumn.isMapped}</span>
                            </div>
                        </div>
                    {/if}
                </div>
                                    ...
                                    <div class="values-list">
                                        {#each selectedColumn.values.slice(0, 5) as value, i}
                                            <div class="value-item">
                                                ...
                                                <span class="property-value property-{getValueType(value)}">{formatValue(value)}</span>
                                            </div>
                                        {/each}
                                    </div>
                                </div>
                ...
                {#if selectedColumn.validation}
                    <div class="property-section">
                        ...
                        {#if expandedSections.validation}
                            <div class="property-list">
                                <div class="property-item">
                                    <span class="property-name">Is Valid:</span>
                                    <span class="property-value property-{selectedColumn.validation.isValid}">{selectedColumn.validation.isValid}</span>
                                </div>
                                
                                {#if selectedColumn.validation.message}
                                    <div class="property-item">
                                        <span class="property-name">Message:</span>
                                        <span class="property-value">{selectedColumn.validation.message}</span>
                                    </div>
                                {/if}
                                ...
                                {#if selectedColumn.validation.errors && selectedColumn.validation.errors.length > 0}
                                    <div class="property-item">
                                        ...
                                        <div class="values-list">
                                            {#each selectedColumn.validation.errors as error, i}
                                                <div class="value-item">
                                                    <span class="value-index">[{i}]:</span>
                                                    <span class="property-value property-string">{error}</span>
                                                </div>
                                            {/each}
                                        </div>
                                    </div>
                ...
                {#if selectedColumn.cellValidation && selectedColumn.cellValidation.length > 0}
                    <div class="property-section">
                        ...
                        {#if expandedSections.cellValidation}
                            <div class="property-list">
                                <div class="property-item">
                                    <span class="property-name">Invalid Cells:</span>
                                    <span class="property-value">
                                        {selectedColumn.cellValidation.filter(v => !v.isValid).length} of {selectedColumn.cellValidation.length}
                                    </span>
                                </div>
                                
                                <div class="property-item">
                                    <span class="property-name">First 5 Cells:</span>
                                    <div class="values-list">
                                        {#each selectedColumn.cellValidation.slice(0, 5) as validation, i}
                                            <div class="value-item">
                                                <span class="value-index">[{i}]:</span>
                                                <span class="property-value property-{validation.isValid}">
...
<style>
    ...
    .columns-overview th, .columns-overview td {
        padding: 8px;
        text-align: left;
        border-bottom: 1px solid #ddd;
    }
